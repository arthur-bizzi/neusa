# This module contains utilities used by some models.
# this is taken from github.com/AdityaLab/pinnsformer

# see "PINNsFormer: A Transformer-Based Framework For Physics-Informed Neural Networks."
...
def make_time_sequence(src, num_step=5, step=1e-4):
    dim = num_step
    src = src.unsqueeze(1).repeat(1, dim, 1)  
    for i in range(num_step):
        src[:, i, -1] += step * i
    return src

def make_sequence_in_first_coord(src, num_step=5, step=1e-4):
    dim = num_step
    src = src.unsqueeze(1).repeat(1, dim, 1)
    for i in range(num_step):
        src[:, i, 0] += step * i
