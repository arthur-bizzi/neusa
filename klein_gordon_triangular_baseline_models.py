...
from utilities.grid import Regular2DGrid
...
def loss_function(model, training_grid, device, u0):
    ...
    loss_res = torch.mean((u_tt - u_xx  + 10*torch.sin(u))**2)

    return (loss_ic, loss_bc, loss_res)

if __name__ == "__main__":

    parser = argparse.ArgumentParser('Klein-Gordon triangular baseline models')
    ...
    save_dir = os.path.join("results", "klein_gordon_triangular", dir_name, model_name, f"seed_{seed}")
    ...
    # initial condition
    u0 = triangle(training_grid.x).to(device)
    ...
    trainer = AdamTrainer(params=list(model.parameters()),
                    ...
                    loss_function=lambda: loss_function(model, training_grid, device, u0)
                )
    ...
    data_loader = KleinGordonTriangular1DGroundTruthLoader(model_name)
