...
if __name__ == "__main__":
    ...
    # basis and grid
    basis = TchebychevBasis2D(M,N,domain_x,domain_y,device=device)
    ...
    # initial condition
    u0 = (torch.sin(np.pi*xs)*torch.sin(np.pi*ys)).squeeze(-1).to(device)
    v0 = (torch.cos(np.pi*xs)*torch.cos(np.pi*ys)).squeeze(-1).to(device)
    hu0 = basis.dbt(u0)
    hv0 = basis.dbt(v0)
    huv0 = torch.cat([hu0,hv0],dim=-1)

    # initial vector field
    F0 = LaplacianSpectralLayer2D(basis.D2x, basis.D2y)
    ...
    NODE = NeuralODE(F,solver='tsit5',sensitivity='autograd',atol=1e-6, rtol=1e-6, return_t_eval=False)
